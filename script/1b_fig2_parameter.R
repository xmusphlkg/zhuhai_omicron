...
# datafile_BA2 <- openxlsx::read.xlsx('./data/omicron_BA2.xlsx')
# datafile_BA2 <- datafile_BA2 |> 
#   mutate_at(vars(contains('date')), convertToDate)
# datafile_Delta <- openxlsx::read.xlsx('./data/Delta.xlsx')
# datafile_Delta <- datafile_Delta |> 
#   mutate_at(vars(contains('date')), convertToDate)
...
datafile_Delta_ib <- datafile_Delta %>% 
  dplyr::select(id, dateonset, infectdate, type) %>% 
  filter(type != 'Asymptomatic' & !is.na(infectdate))
...
datafile_cont_2 <- datafile_BA2 |> 
  dplyr::select(id, infector, dateonset)

datafile_cont_2 <- datafile_cont_2 |> 
  rename('infectee_date' = dateonset) |> 
  left_join(datafile_cont_2, by = c('infector' = 'id')) |> 
  rename('infector_date' = dateonset) |> 
  mutate(date_sep = infectee_date - infector_date) |> 
  filter(date_sep >=0)

datafile_cont_3 <- datafile_Delta |> 
  dplyr::select(id, infector, dateonset)

datafile_cont_3 <- datafile_cont_3 |> 
  rename('infectee_date' = dateonset) |> 
  left_join(datafile_cont_3, by = c('infector' = 'id')) |> 
  rename('infector_date' = dateonset) |> 
  mutate(date_sep = infectee_date - infector_date) |> 
  filter(date_sep >=0)
...
data_onset_1 <- datafile_info_clean %>%
  group_by(dateonset) %>%
  count() %>%
  rename(c(
    'cases' = 'n',
...
# gt_max_ba1 <- fitdistr(as.numeric(datafile_cont_1$gt_max), "gamma")
# gt_min_ba1 <- fitdistr(as.numeric(datafile_cont_1$gt_min + 0.002), "gamma")
gt_ba1 <- fitdistr(as.numeric(datafile_cont_1$gt_median + 0.002), "gamma")

data_expose <- datafile_BA2 |> 
  dplyr::select(id, infector, dateexpose1, dateexpose2)

datafile_cont_2 <- data_expose |> 
  rename(c('infectee_exposedate1' = dateexpose1,
           'infectee_exposedate2' = dateexpose2)) |> 
  left_join(data_expose, by = c('infector' = 'id')) |> 
  rename(c('infector_exposedate1' = dateexpose1,
           'infector_exposedate2' = dateexpose2)) |> 
  filter(!is.na(infector_exposedate1))
...
# gt_max_ba2 <- fitdistr(as.numeric(datafile_cont_2$gt_max), "gamma")
# gt_min_ba2 <- fitdistr(as.numeric(datafile_cont_2$gt_min + 0.002), "gamma")
gt_ba2 <- fitdistr(as.numeric(datafile_cont_2$gt_median + 0.002), "gamma")

data_expose <- datafile_Delta |> 
  dplyr::select(id, infector, infectdate)

datafile_cont_3 <- data_expose |> 
  rename(c('infectee_infectdate' = infectdate)) |> 
  left_join(data_expose, by = c('infector' = 'id')) |> 
  rename(c('infector_infectdate' = infectdate)) |> 
  filter(!is.na(infector_infectdate) & !is.na(infectee_infectdate)) |> 
  mutate(gt_median = as.numeric(infectee_infectdate - infector_infectdate),
...
datafile_positive <- datafile_BA2 %>% 
  dplyr::select(id, infector, datepositive)

datafile_cont_2 <- datafile_positive %>% 
  rename('infectee_date' = datepositive) |> 
  left_join(datafile_positive, by = c('infector' = 'id')) |> 
  rename('infector_date' = datepositive) |> 
  mutate(date_sep = infectee_date - infector_date) %>% 
...
datafile_positive <- datafile_Delta %>% 
  dplyr::select(id, infector, datepositive)

datafile_cont_3 <- datafile_positive %>% 
  rename('infectee_date' = datepositive) |> 
  left_join(datafile_positive, by = c('infector' = 'id')) |> 
  rename('infector_date' = datepositive) |> 
  mutate(date_sep = infectee_date - infector_date) %>% 
...
fig_ib <- fig_ib + inset_element(fig_ib_inside, 0.5, 0.2, 1, 1)
...
fig_si <- fig_si + 
  inset_element(fig_si_inside, 0.5, 0.2, 1, 1)
...
fig_tg <- fig_tg + 
  inset_element(fig_tg_inside, 0.5, 0.2, 1, 1)
...
fig_gt <- fig_gt + 
  inset_element(fig_gt_inside, 0.5, 0.2, 1, 1)
...
# ggsave(filename = './outcome/publish/Figure 21.pdf', 
#        device = cairo_pdf, height = 10, width = 8)
