...
datafile_household_mix <- datafile_cont_all %>% 
  mutate(vaccine_g = factor(vaccine, levels = 0:3,
                            labels = c(0, 0, 1, 2)),
         vaccine_mix = if_else(is.na(vaccine_mix),
                               ...
                               vaccine_mix)) %>% 
  filter(contact == '同住') %>%
  group_by(vaccine_g, vaccine_mix, outcome) %>% 
  count() %>% 
  group_by(vaccine_g, vaccine_mix) %>% 
  mutate(freq = round(n/sum(n), digits = 4),
         type = 'Household') |> 
  ungroup()
...
datafile_outer_mix <- datafile_cont_all %>% 
  mutate(vaccine_g = factor(vaccine, levels = 0:3,
                            labels = c(0, 0, 1, 2)),
         vaccine_mix = if_else(is.na(vaccine_mix),
                               'N',
...
datafile_mix <- rbind(datafile_household_mix, datafile_outer_mix) %>% 
